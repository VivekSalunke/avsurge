...
export default function BulkImportPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10 text-[var(--text)]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Bulk Import</h1>
          <p className="text-sm text-[rgba(255,255,255,0.4)]">Duplicates detected by exact slug + strict name matching</p>
        </div>
        <Link href="/admin" className="text-sm text-dim hover:text-[rgba(255,255,255,0.85)] px-3 py-2">← Admin</Link>
      </div>
      ...
      {status === 'error' && (
        <div className="bg-[rgba(239,68,68,0.06)] border border-[rgba(239,68,68,0.2)] text-[#f87171] rounded-xl px-4 py-3 text-sm mb-4">{message}</div>
      )}
      {(status === 'importing' || status === 'checking') && (
        <div className="bg-[rgba(6,182,212,0.06)] border border-[rgba(6,182,212,0.2)] text-neon-cyan rounded-xl px-4 py-3 text-sm mb-4">
          {status === 'checking' ? 'Checking for duplicates...' : `Importing... ${imported} done`}
        </div>
      )}

      {showDuplicates && duplicates.length > 0 && (
        <div className="bg-[rgba(251,191,36,0.06)] border border-[rgba(251,191,36,0.2)] rounded-xl p-4 mb-4">
          ...
          <div className="flex flex-col gap-2 mb-4 max-h-48 overflow-y-auto">
            {duplicates.map((d, i) => (
              <div key={i} className="flex items-center gap-2 text-xs bg-[rgba(255,255,255,0.02)] border border-[rgba(251,191,36,0.2)] rounded-lg px-3 py-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2.5 border border-[rgba(255,255,255,0.1)] text-dim rounded-xl text-sm hover:bg-[rgba(255,255,255,0.02)] hover:text-white transition">
              Cancel
            </button>
          </div>
        </div>
      ...
      <textarea
        className="w-full h-96 border border-[rgba(255,255,255,0.06)] rounded-2xl px-4 py-3 text-xs font-mono focus:outline-none focus:border-neon-cyan mb-4 resize-none"
        value={json}
