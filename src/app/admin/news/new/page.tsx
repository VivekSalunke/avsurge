      ...
      <div className="bg-[var(--card-bg)] border border-[rgba(255,255,255,0.06)] rounded-2xl p-6 space-y-4 neon-border">
        <div>
          ...
          <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value, slug: autoSlug(e.target.value) }))}
            placeholder="Article title" className="w-full border border-[rgba(255,255,255,0.06)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-neon-cyan" style={inputStyle} />
        </div>
        <div>
          <label className="text-xs text-dim mb-1 block">Slug *</label>
          <input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))}
            placeholder="article-slug" className="w-full border border-[rgba(255,255,255,0.06)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-neon-cyan" style={inputStyle} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input value={form.image_url} onChange={e => setForm(f => ({ ...f, image_url: e.target.value }))}
              placeholder="https://..." className="w-full border border-[rgba(255,255,255,0.06)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-neon-cyan" style={inputStyle} />
          </div>
        </div>
        <div>
          <label className="text-xs text-dim mb-1 block">Excerpt (shown in listing)</label>
          <textarea value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))}
            placeholder="Brief description of the article..." rows={2}
            className="w-full border border-[rgba(255,255,255,0.06)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-neon-cyan resize-none" style={inputStyle} />
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button onClick={() => handleSave(false)} disabled={saving}
            className="border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.02)] text-[rgba(255,255,255,0.85)] rounded-xl px-5 py-2.5 text-sm font-semibold hover:border-neon-cyan hover:text-white transition disabled:opacity-50">
            Save as Draft
          </button>
        </div>
      </div>
