...
export default async function BrandPage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  const totalDevices = (phones?.length || 0) + (tablets?.length || 0)
...
export async function generateMetadata({ params }: { params: Promise<{ brand: string }> }) {
  ...
  const total = (phones?.length || 0) + (tablets?.length || 0)
  return {
    title: `${brandName} Phones & Tablets Price List in India`,
    description: `Browse all ${brandName} smartphones and tablets in India. ${total} ${brandName} devices available with specs and prices.`,
