...
export default async function BrandsPage() {
  const [{ data: phonesRaw }, { data: tabletsRaw }] = await Promise.all([
  ...
  for (const p of phonesRaw || []) {
    phoneBrandCount[p.brand] = (phoneBrandCount[p.brand] || 0) + 1
  }

  const tabletBrandCount: Record<string, number> = {}
  for (const t of tabletsRaw || []) {
    tabletBrandCount[t.brand] = (tabletBrandCount[t.brand] || 0) + 1
  }
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-400">Browse phones and tablets by brand</p>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {tabletBrands.map(([brand, count]) => (
            <Link key={brand} href={`/tablets?brand=${encodeURIComponent(brand)}`}
              className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-blue-300 hover:shadow-sm transition group text-center">
              <div className="text-4xl mb-3">{brandIcons[brand] || '📟'}</div>
              <h3 className="font-bold text-gray-900 group-hover:text-blue-600 transition text-sm">{brand}</h3>
              <p className="text-xs text-gray-400 mt-1">{count} tablets</p>
            </Link>
          ))}
        </div>
      </div>
