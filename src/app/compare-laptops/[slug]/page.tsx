  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="text-sm text-gray-400 mb-6 flex items-center gap-1.5">
        ...
        <span>&rsaquo;</span>
        <span className="text-gray-600">{laptopA.name} vs {laptopB.name}</span>
      </div>

      <h1 className="text-2xl font-bold text-gray-900 mb-2">{laptopA.name} vs {laptopB.name}</h1>
      <p className="text-sm text-gray-400 mb-8">Full specs comparison, price and performance details</p>

      <div className="grid grid-cols-2 gap-4 mb-8">
        {[laptopA, laptopB].map((laptop) => (
          <div key={laptop.id} className="bg-white border border-gray-200 rounded-2xl p-5 text-center">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4 flex items-center justify-between flex-wrap gap-3 mb-8">
        <div>
          <p className="text-xs text-blue-400 mb-0.5">Want to compare with a different laptop?</p>
        ...
        {allCategories.map(cat => (
          <div key={cat} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="grid grid-cols-3 border-b border-gray-100">
              <div className="px-4 py-2 text-xs font-medium text-blue-600 bg-blue-50/50 truncate">{laptopA.name}</div>
              <div className="px-4 py-2 text-xs font-medium text-gray-400 text-center">Spec</div>
              <div className="px-4 py-2 text-xs font-medium text-purple-600 bg-purple-50/50 text-right truncate">{laptopB.name}</div>
            </div>
