...
export default function TabletPriceHistory({ tabletId, currentPrice }: { tabletId: number, currentPrice: number | null }) {
  const [history, setHistory] = useState<PriceEntry[]>([])

  const queryHistory = useCallback(() => supabase
    .from('tablet_price_history')
    .select('*')
    .eq('tablet_id', tabletId)
    .order('tracked_at', { ascending: true }), [tabletId])
      ...
      <div className="px-5 py-4">
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="bg-[rgba(255,255,255,0.02)] rounded-xl p-3 text-center">
            <p className="text-xs text-[rgba(255,255,255,0.4)] mb-1">Lowest</p>
            <p className="text-sm font-bold text-green-400">{formatPriceINR(minPrice)}</p>
          </div>
          <div className="bg-[rgba(255,255,255,0.02)] rounded-xl p-3 text-center">
            <p className="text-xs text-[rgba(255,255,255,0.4)] mb-1">Current</p>
            <p className="text-sm font-bold text-neon-cyan">{formatPriceINR((currentPrice || latestPrice))}</p>
          </div>
          <div className="bg-[rgba(255,255,255,0.02)] rounded-xl p-3 text-center">
            <p className="text-xs text-[rgba(255,255,255,0.4)] mb-1">Highest</p>
          ...
          {history.map((entry, i) => {
            ...
            return (
              <div key={entry.id} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                <div
                  className={`w-full rounded-t-md ${isLatest ? 'bg-neon-cyan' : 'bg-[rgba(6,182,212,0.25)] group-hover:bg-[rgba(6,182,212,0.45)]'} transition-all`}
                ...
                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 group-hover:opacity-100 transition pointer-events-none z-10">
                  ...
                  <br />
                  <span className="text-[rgba(255,255,255,0.4)]">{new Date(entry.tracked_at).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}</span>
                </div>
              </div>
            )
          })}
